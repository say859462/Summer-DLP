...
class MaskGIT:
    ...
    def inpainting(self, image, mask_b, i):  # MakGIT inference
        ...
        maska = torch.zeros(
            self.total_iter, 3, 16, 16,device = self.device
        )  # save all iterations of masks in latent domain
        imga = torch.zeros(
            self.total_iter + 1, 3, 64, 64,device = self.device
        ...
        self.model.eval()
        with torch.no_grad():
            # Batch size = 1 , we can unsqueeze the first dimension
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--load-transformer-ckpt-path", type=str, default="./transformer_checkpoints/best_val.pth", help="load ckpt"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="./lab3_dataset/masked_image",
        help="Path to testing image dataset.",
    )
    parser.add_argument(
        "--test-mask-path",
        type=str,
        default="./lab3_dataset/mask64",
